function searchHelpRequest() {
  ...
  xhttp.open(
    "POST",
    "../../controller/admin_controller/searchHelpRequest.php",
    true
  );
...
function viewPendingHelpRequest() {
  ...
  xhttp.open(
    "POST",
    "../../controller/admin_controller/viewPendingHelpRequest.php",
    true
  );
  xhttp.send();
...
function viewResolvedHelpRequest() {
  ...
  xhttp.open(
    "POST",
    "../../controller/admin_controller/viewResolvedHelpRequest.php",
    true
  );
  xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhttp.send();
    ...
    helpRequests.forEach((request, index) => {
      let row = `
        <tr>
          ...
          <td style='text-align: center;'>${request.category}</td>
          <td colspan='2' style='text-align: justify;'>${request.query}</td>
          <td colspan='2' style='text-align: justify;'>${request.feedback}</td>
        </tr>`;
      table.innerHTML += row;
    });
  } else {
